...
function handleFileUpload(event, type) {
  ...
  reader.onload = function(e) {
    ...
    showToast(`${type === 'profileImage' ? 'Profile image' : 'Logo'} uploaded successfully!`);
  };
...
function resetForm() {
  if (confirm('Are you sure you want to reset the form? All your data will be lost.')) {
    ...
    uploadedImages = { profileImage: null, logo: null };
    
    // Reset color picker
    document.getElementById('themeColor').value = '#3b82f6';
    ...
    // Clear all dynamic items except one each
    ['services', 'projects', 'experience', 'education', 'testimonials'].forEach(type => {
      ...
      container.innerHTML = '';
      if (type === 'services') addService();
      if (type === 'projects') addProject();
      if (type === 'experience') addExperience();
      if (type === 'education') addEducation();
      if (type === 'testimonials') addTestimonial();
    });
...
function addProject() {
  ...
  div.innerHTML = `
    ...
    <div class="file-upload">
      ...
      <span class="project-image-name">No file chosen</span>
  ...
  const fileInput = div.querySelector('.project-image');
  fileInput.addEventListener('change', function(e) {
    ...
    if (file) {
      ...
      reader.onload = function(e) {
        ...
        div.querySelector('.project-image-name').textContent = file.name;
      };
      reader.readAsDataURL(file);
    }
  });
...
function addTestimonial() {
  ...
  div.innerHTML = `
    ...
    <div class="file-upload">
      ...
      <span class="testimonial-image-name">No file chosen</span>
      <button type="button" class="btn-upload">Choose File</button>
    </div>
  ...
  // Add event listener for the new file input
  const fileInput = div.querySelector('.testimonial-image');
  fileInput.addEventListener('change', function(e) {
    const file = e.target.files[0];
    if (file) {
      const reader = new FileReader();
      reader.onload = function(e) {
        fileInput.dataset.imageData = e.target.result;
        div.querySelector('.testimonial-image-name').textContent = file.name;
      };
      reader.readAsDataURL(file);
    }
  });
...
function saveToLocalStorage() {
  ...
  const projectImages = {};
  document.querySelectorAll('#projects-container .array-item').forEach((item, index) => {
    const fileInput = item.querySelector('.project-image');
    if (fileInput && fileInput.dataset.imageData) {
      projectImages[`project${index}`] = fileInput.dataset.imageData;
    }
  });
  
  // Gather all testimonial images
  const testimonialImages = {};
  document.querySelectorAll('#testimonials-container .array-item').forEach((item, index) => {
    const fileInput = item.querySelector('.testimonial-image');
    if (fileInput && fileInput.dataset.imageData) {
      testimonialImages[`testimonial${index}`] = fileInput.dataset.imageData;
    }
  });
  ...
  const data = {
    ...
    profileImage: uploadedImages.profileImage ? uploadedImages.profileImage.data : '',
    logo: uploadedImages.logo ? uploadedImages.logo.data : '',
    ...
    sections: {
      ...
      services: document.getElementById('toggle-services').checked,
    ...
    // Projects
    projects: Array.from(document.querySelectorAll('#projects-container .array-item')).map((item, index) => ({
      ...
      image: projectImages[`project${index}`] || '',
    ...
    // Testimonials
    testimonials: Array.from(document.querySelectorAll('#testimonials-container .array-item')).map((item, index) => ({
      name: item.querySelector('.testimonial-name').value,
      position: item.querySelector('.testimonial-position').value,
      image: testimonialImages[`testimonial${index}`] || '',
...
function loadFromLocalStorage() {
  ...
  if (data.profileImage) {
    uploadedImages.profileImage = { data: data.profileImage };
    document.getElementById('profileImageName').textContent = 'Uploaded image';
  }
  
  if (data.logo) {
    uploadedImages.logo = { data: data.logo };
    document.getElementById('logoName').textContent = 'Uploaded logo';
  }
  
  // Section toggles
  if (data.sections) {
    document.getElementById('toggle-about').checked = data.sections.about;
    document.getElementById('toggle-skills').checked = data.sections.skills;
    document.getElementById('toggle-services').checked = data.sections.services;
    document.getElementById('toggle-projects').checked = data.sections.projects;
    document.getElementById('toggle-experience').checked = data.sections.experience;
    document.getElementById('toggle-testimonials').checked = data.sections.testimonials;
    
    // Update section visibility
    document.getElementById('skills-group').style.display = data.sections.skills ? 'block' : 'none';
    document.getElementById('services-group').style.display = data.sections.services ? 'block' : 'none';
    document.getElementById('projects-group').style.display = data.sections.projects ? 'block' : 'none';
    document.getElementById('experience-group').style.display = data.sections.experience ? 'block' : 'none';
    document.getElementById('testimonials-group').style.display = data.sections.testimonials ? 'block' : 'none';
  }
  ...
  if (data.projects && data.projects.length > 0) {
    data.projects.forEach((project, index) => {
      ...
      if (project.image) {
        const fileInput = lastItem.querySelector('.project-image');
        fileInput.dataset.imageData = project.image;
        lastItem.querySelector('.project-image-name').textContent = 'Uploaded image';
      }
    });
  } else {
    addProject();
  ...
  if (data.testimonials && data.testimonials.length > 0) {
    data.testimonials.forEach((testimonial, index) => {
      ...
      if (testimonial.image) {
        const fileInput = lastItem.querySelector('.testimonial-image');
        fileInput.dataset.imageData = testimonial.image;
        lastItem.querySelector('.testimonial-image-name').textContent = 'Uploaded image';
      }
    });
  } else {
    addTestimonial();
...
function generatePortfolio() {
  ...
  // Get profile image (use uploaded data if available)
  let profileImage = 'https://via.placeholder.com/400x400';
  if (uploadedImages.profileImage) {
    profileImage = uploadedImages.profileImage.data;
  }
  
  // Get logo (use uploaded data if available)
  let logo = '';
  if (uploadedImages.logo) {
    logo = uploadedImages.logo.data;
  }
  ...
  if (sections.projects) {
    document.querySelectorAll('#projects-container .array-item').forEach(item => {
      const fileInput = item.querySelector('.project-image');
      const imageData = fileInput ? fileInput.dataset.imageData : '';
  ...
  // Education
  const education = [];
  if (sections.experience) { // Education is part of experience section
  ...
  if (sections.testimonials) {
    document.querySelectorAll('#testimonials-container .array-item').forEach(item => {
      ...
      testimonials.push({
        ...
        image: imageData || 'https://randomuser.me/api/portraits/men/32.jpg',
...
<body>
  ...
  <header id="header">
    <div class="container header-inner">
      ...
      <nav class="nav-links">
        <a href="#home">Home</a>
        ${sections.about ? '<a href="#about">About</a>' : ''}
      ...
      <div class="hero-content">
        <div class="hero-text fade-in" style="animation-delay: 0.1s;">
          ...
          <div class="hero-buttons">
            ...
            ${sections.projects ? '<a href="#projects" class="btn btn-outline">View Work</a>' : ''}
          </div>
        </div>
  ...
  <section id="experience" class="experience">
    <div class="container">
      ...
      <div class="timeline">
        ...
        ${education.length > 0 ? `
        <div class="timeline-item">
          <div class="timeline-dot">${experience.length + 1}</div>
          <div class="timeline-content">
            <h3>Education</h3>
            ${education.map(edu => `
              <h4>${edu.degree}</h4>
...
async function downloadPortfolio() {
  try {
    // First generate the portfolio to ensure we have the latest content
    generatePortfolio();
    
    // Get the HTML content from the preview frame
...
<body>
  ${doc.body.innerHTML}
    ...
    if (uploadedImages.profileImage) {
      const profileImgData = uploadedImages.profileImage.data.split(',')[1];
      folder.file("images/profile.jpg", profileImgData, {base64: true});
    }
    
    if (uploadedImages.logo) {
      const logoData = uploadedImages.logo.data.split(',')[1];
      folder.file("images/logo.png", logoData, {base64: true});
    }
    ...
    document.querySelectorAll('#projects-container .array-item').forEach((item, index) => {
      const fileInput = item.querySelector('.project-image');
      if (fileInput && fileInput.dataset.imageData) {
        const imgData = fileInput.dataset.imageData.split(',')[1];
        folder.file(`images/project${index + 1}.jpg`, imgData, {base64: true});
      }
    });
    ...
    document.querySelectorAll('#testimonials-container .array-item').forEach((item, index) => {
      ...
      if (fileInput && fileInput.dataset.imageData) {
        const imgData = fileInput.dataset.imageData.split(',')[1];
        folder.file(`images/testimonial${index + 1}.jpg`, imgData, {base64: true});
      }
    });
